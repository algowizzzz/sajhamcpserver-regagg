---
title: "Directive sur la soumission de déclarations d'opérations douteuses dans le format de déclaration de première génération (Génération 1)"
regulator: "fintrac"
doc_type: "announcement"
status: "final"
source_kind: "web"
source_url: "https://fintrac-canafe.canada.ca/reporting-declaration/fwr/fwr-str-fra.php"
version: "1"
---
# Directive sur la soumission de déclarations d'opérations douteuses dans le format de déclaration de première génération (Génération 1)
...
Lors de la saisie d'informations dans le [Système de déclaration en ligne de CANAFE](https://www142.fintrac-canafe.canada.ca/), veuillez-vous référer au Tableau de correspondance des champs à l’annexe A, qui fournit l’équivalence pour les champs clés du formulaire dans le nouveau système de déclaration.
...
* **Numéro de référence de l'opération de l'entité déclarante :**
  ...
  Il s'agit d'un numéro unique attribué à chaque opération par vous (l'entité déclarante) ou par l'individu ou l'organisation soumettant le rapport en votre nom.
* **Numéro d'emplacement de l’entité déclarante :**

  Cela représente des informations sur le lieu où l'opération a eu lieu. Par exemple, si le client a déposé de l'argent en espèces à la Succursale 1, sélectionnez le numéro d'emplacement associé à la Succursale 1. Si le client effectue une opération en ligne, sélectionnez le numéro d'emplacement associé au lieu qui traite les instructions en ligne du client – cela pourrait être un emplacement de succursale ou le siège social, selon votre processus d'affaires.

  Le numéro d'emplacement est créé et attribué par CANAFE lors du processus d'inscription au Système de déclaration en ligne de CANAFE et est maintenu par votre administrateur.

  Pour l’instant, vous n'aurez pas la capacité d'ajouter de nouveaux emplacements ou de modifier des emplacements existants dans le Système de déclaration en ligne de CANAFE. Si une opération déclarée a lieu dans un nouvel emplacement, veuillez utiliser le numéro de localisation de votre siège social pour le moment.
* **Type d’opération douteuse :** veuillez vous référer aux instructions des champs sous « détails de l'opération douteuse » dans la directive de CANAFE : [Déclaration d'opérations douteuses à CANAFE](/guidance-directives/transaction-operation/str-dod/str-dod-fra#s11)
...
| STR.A.01 | Numéro d'identification de la personne ou de l'entité déclarante \*   (le cas échéant) | \* Numéro d'identification de l'entité déclarante |
...
| STR.A.03 | Adresse (rue et numéro)\* | \* Numéro d'emplacement de l'entité déclarante dans la section Renseignements sur l'emplacement où l'opération a été effectuée  **Mesure intérimaire :** si l’opération a eu lieu à un nouvel emplacement, sélectionnez l’emplacement de votre siège social |
| STR.A.04 | Ville\* |
| STR.A.05 | Province \* |
| STR.A.05 | Province \* |
...
| STR.B1.09 | Numéro de compte de l'autre institution, entité ou personne\*   (le cas échéant) | Saisir dans « † Numéro de compte » dans l’action d’amorce |
| Action d’achèvement | | |
...
| STR.C.08 | Statut du compte au moment où l’opération a été amorcée\*   (le cas échéant) | † Statut du compte au moment de l'opération |
...
| STR.D.04 | Numéro de client attribué par la personne ou l’entité déclarante\*   (le cas échéant) |
...
| STR.E.04 | Ville | Saisir dans les champs correspondants de la section Renlseignements au sujet de ’entité pour le compte de qui l’opération ou la tentative d’opération a été effectuée (le cas échéant) |
...
| STR.G.01 | \* Veuillez décrire clairement tous les facteurs ou circonstances inhabituelles qui vous ont amené à avoir des soupçons relativement au blanchiment d´argent ou au financement d´activités terroristes.    Expliquez ce que vous trouvez douteux en donnant le plus de détails possibles.    Si cette déclaration concerne une ou plusieurs opérations qui ont été tentées, décrivez également les raisons pour lesquelles chacune d'entre elles n'a pas abouti. | \* Veuillez décrire en termes clairs, simples et concis vos motifs de soupçonner une infraction de blanchiment d’argent ou de financement d’activités terroristes, y compris les faits, le contexte et les indicateurs qui vous ont permis d’établir un motif raisonnable de soupçonner une telle opération. |
...
* **Les champs obligatoires** sont indiqués par un symbole astérisque (\*) et doivent être complétés. Cependant, dans le cas d'une transaction tentée, vous devez prendre des mesures raisonnables pour obtenir les informations pour tout champ obligatoire.
* **Obligatoire si applicable :** Les champs qui comportent à la fois un astérisque et la mention « si applicable » à côté doivent être complétés s'ils vous concernent ou concernent la transaction rapportée.

## Instructions pour les catégories de champs (Génération 2)

* **Les champs obligatoires** sont indiqués par un symbole astérisque (\*) et doivent être complétés. Cependant, **dans le cas d'une opération tentée**, ces champs deviennent des champs de « mesures raisonnables » et vous devez prendre des mesures raisonnables, tel qu'indiqué dans les instructions ci-dessous, pour obtenir les informations pour tout champ obligatoire.
* **Les champs obligatoires pour le traitement** sont indiqués par un symbole double obèle (‡) et doivent être complétés.
* **Les champs obligatoires si applicable** sont indiqués par un symbole obèle (†) et doivent être complétés s'ils vous concernent ou concernent l'opération rapportée.
* Vous devez prendre des [mesures raisonnables](/guidance-directives/glossary-glossaire/1-fra#raisonnable) pour obtenir les informations pour tous les champs non obligatoires dans le rapport, **s'ils vous concernent**.
